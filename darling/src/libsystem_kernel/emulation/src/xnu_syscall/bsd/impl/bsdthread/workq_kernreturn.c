#include <emulation/xnu_syscall/bsd/impl/bsdthread/workq_kernreturn.h>
...
#include <emulation/linux_api/c_api/futex.h>
	...
	switch (options)
	{
		case WQOPS_THREAD_KEVENT_RETURN:
		{
			...
			if (wq_event_pending != NULL)
			{
				int* sem;
				if (nkevents > 0)
					memmove(wq_event_pending->events, keventlist, nkevents * sizeof(struct bsd_kevent_qos_s));
