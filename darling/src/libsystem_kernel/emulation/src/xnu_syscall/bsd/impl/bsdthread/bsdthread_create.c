...
#include <emulation/conversion/variables/bsd/sys/mman.h>
...
#include <emulation/xnu_syscall/mach/mach_syscall_table.h>
#include <emulation/xnu_syscall/machdep/impl/tls.h>
...
void* thread_stack_allocate(unsigned long stacksize)
{
	...
	stack = (void**) sys_mmap(NULL, allocsize, BSD_PROT_READ | BSD_PROT_WRITE,
			BSD_MAP_ANON | BSD_MAP_PRIVATE, -1, 0);

	// Protect the stack guard
	sys_mprotect(stack, STACK_GUARD_SIZE, BSD_PROT_NONE);
