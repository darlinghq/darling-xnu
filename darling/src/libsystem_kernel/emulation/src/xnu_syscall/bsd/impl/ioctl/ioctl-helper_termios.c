...
#include <emulation/conversion/variables/bsd/sys/ioctl_compat.h>
#include <emulation/conversion/variables/bsd/sys/termios.h>
#include <emulation/conversion/variables/bsd/sys/ttycom.h>
#include <emulation/conversion/variables/bsd/sys/unistd.h>
#include <emulation/conversion/variables/linux/unistd.h>
#include <emulation/conversion/variables/linux/asm/termbits.h>
#include <emulation/conversion/variables/linux/asm-generic/ioctls.h>
...
static void sgflag_linux_to_bsd(const struct linux_termios2* tos, short* sgflags);
static void sgflag_bsd_to_linux(const short sgflags, struct linux_termios2* tos);
...
int handle_termios(int fd, unsigned int cmd, void* arg, int* retval)
{
	switch (cmd)
	{
		case BSD_TIOCGETA:
		{
			...
			struct linux_termios2 in;

			*retval = __real_ioctl(fd, LINUX_TCGETS2, &in);
		...
		{
			struct linux_termios2 in;
			struct bsd_sgttyb* out = (struct bsd_sgttyb*) arg;
		...
		case BSD_TIOCSETP:
		{
			...
			*retval = __real_ioctl(fd, LINUX_TCSETS2, &out);
		...
		{
			struct linux_termios2 in;
			struct bsd_tchars* out = (struct bsd_tchars*) arg;
		...
		{
			struct linux_termios2 out;
			const struct bsd_tchars* in = (const struct bsd_tchars*) arg;
		...
		{
			struct linux_termios2 in;
			struct bsd_ltchars* out = (struct bsd_ltchars*) arg;
		...
		{
			struct linux_termios2 out;
			const struct bsd_ltchars* in = (const struct bsd_ltchars*) arg;
		...
		case BSD_TIOCIXOFF:
		{
			struct linux_termios2 tos;
			unsigned char c;

			*retval = __real_ioctl(fd, LINUX_TCGETS2, &tos);
			if (*retval < 0)
				return IOCTL_HANDLED;

			c = tos.c_cc[cmd == BSD_TIOCIXON ? LINUX_VSTART : LINUX_VSTOP];
			if (c != LINUX__POSIX_VDISABLE)
		...
		case BSD_TIOCISATTY:
		{
			...
			*retval = __real_ioctl(fd, LINUX_TCGETS2, &in) == 0;
			return IOCTL_HANDLED;
		}
		default:
		{
