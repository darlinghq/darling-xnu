...
#include <emulation/conversion/variables/linux/asm/mman.h>
#include <emulation/conversion/variables/linux/linux/mman.h>
...
static int prot_bsd_to_linux(int prot)
{
	...
	if (prot & BSD_PROT_READ)
		linux_prot |= LINUX_PROT_READ;
	if (prot & BSD_PROT_WRITE)
		linux_prot |= LINUX_PROT_WRITE;
	if (prot & BSD_PROT_EXEC)
