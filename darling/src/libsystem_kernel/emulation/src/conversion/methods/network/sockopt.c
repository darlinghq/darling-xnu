...
#include <emulation/conversion/variables/bsd/sys/ucred.h>
...
int sockopt_bsd_to_linux(int* level, int* optname, void** optval, void* optbuf)
{
	...
	if (*optname == BSD_LOCAL_PEERCRED && *level == BSD_IPPROTO_IP)
	{
		struct bsd_xucred* c = (struct bsd_xucred*) optbuf;
		// Simulate euid 0
		memset(c, 0, sizeof(*c));
		*optname = 0;
		return 0;
		...
		*level = LINUX_SOL_SOCKET;
		...
		switch (*optname)
		{
			...
			case BSD_SO_LINGER:
			{
				if (optval != NULL)
				{
					struct bsd_linger *out, *in;
					
					in = (struct bsd_linger*) *optval;
					out = (struct bsd_linger*) optbuf;
			...
			case BSD_SO_NOSIGPIPE:
				*optname = 0; break; // FIXME: send() with MSG_NOSIGNAL...
			default:
				return -BSD_EINVAL;
		}
	}
	else if (*level == BSD_IPPROTO_TCP)
	{
		switch (*optname)
		{
	...
	else if (*level == BSD_IPPROTO_IPV6)
	{
		switch (*optname)
		{
			...
			case BSD_IPV6_JOIN_GROUP:
				*optname = LINUX_IPV6_ADD_MEMBERSHIP; break;
			case BSD_IPV6_LEAVE_GROUP:
				*optname = LINUX_IPV6_DROP_MEMBERSHIP; break;
