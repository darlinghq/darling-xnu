...
#include <emulation/conversion/variables/linux/asm/errno.h>
...
#define dserver_rpc_hooks_iovec_t struct linux_iovec
...
#define dserver_rpc_hooks_atomic_save_t bsd_sigset_t
...
static void dserver_rpc_hooks_atomic_begin(dserver_rpc_hooks_atomic_save_t* atomic_save) {
	...
	bsd_sigset_t set = 0x7fffffff;
	sys_sigprocmask(BSD_SIG_BLOCK, &set, atomic_save);
};

static void dserver_rpc_hooks_atomic_end(dserver_rpc_hooks_atomic_save_t* atomic_save) {
	sys_sigprocmask(BSD_SIG_SETMASK, atomic_save, NULL);
};
