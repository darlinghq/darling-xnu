...
#include <emulation/conversion/variables/bsd/sys/errno.h>
...
struct attrlist* alist, void *attributeBuffer, __SIZE_TYPE__ bufferSize, unsigned long options)
{
	...
	if (alist->dirattr & ATTR_DIR_ENTRYCOUNT) {
		...
		*((uint32_t*)next) = 0;
		...
		if (tmp_fd < 0) {
			rv = -BSD_EIO; // translate all errors into EIO (to avoid confusing our caller)
		...
		while (1) {
			...
			if (rv < 0) {
				rv = -BSD_EIO; // same as before: avoid confusing our caller with unexepected errnos
